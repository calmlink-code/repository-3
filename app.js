...
const RECOMMENDED_BACKUP_PATH = "デスクトップ / ロボット教室共有システム / バックアップ";
...
const seedState = {
  ...
  students: [
    { id: "s1", name: "山田 太郎", classroomId: "c1", classOptionId: "co1", status: "在籍", tuition: 13200, guardian: "山田 花子", phone: "090-1111-1111", email: "yamada@example.com", weekdays: ["火"] },
    { id: "s2", name: "高橋 陽斗", classroomId: "c1", classOptionId: "co2", status: "在籍", tuition: 13200, guardian: "高橋 真理", phone: "090-2222-2222", email: "takahashi@example.com", weekdays: ["木"] },
    { id: "s3", name: "鈴木 花", classroomId: "c1", classOptionId: "co1", status: "在籍", tuition: 13200, guardian: "鈴木 一郎", phone: "090-3333-3333", email: "suzuki-family@example.com", weekdays: ["火", "木"] },
    { id: "s4", name: "伊藤 結菜", classroomId: "c2", classOptionId: "co1", status: "在籍", tuition: 12100, guardian: "伊藤 純子", phone: "090-4444-4444", email: "ito@example.com", weekdays: ["水"] },
    ...
    { id: "s6", name: "小林 凛", classroomId: "c3", classOptionId: "co1", status: "在籍", tuition: 12100, guardian: "小林 明美", phone: "090-6666-6666", email: "kobayashi@example.com", weekdays: ["月"] },
    { id: "s7", name: "加藤 新", classroomId: "c3", classOptionId: "co2", status: "在籍", tuition: 12100, guardian: "加藤 香織", phone: "090-7777-7777", email: "kato@example.com", weekdays: ["金"] },
  ],
  attendance: [
...
function normalizeState(raw) {
  const parsed = structuredClone(raw || seedState);
  return {
    settings: {
      fiscalClosingMonth: Number(parsed.settings?.fiscalClosingMonth || seedState.settings.fiscalClosingMonth || 3),
    },
    classOptions: (parsed.classOptions || seedState.classOptions)
      .map((item, index) => ({
        id: item.id,
        name: item.name,
    ...
    classrooms: (parsed.classrooms || seedState.classrooms).map((classroom) => ({
      id: classroom.id,
      name: classroom.name,
      manager: classroom.manager,
      days: classroom.days || "",
    })),
    users: (parsed.users || seedState.users).map((user, index) => ({
      ...user,
      email: user.email || `user${index + 1}@local.app`,
      password: user.password || "password123",
    })),
    students: (parsed.students || seedState.students).map((student) => ({
      ...student,
      classOptionId: student.classOptionId || "",
      startMonth: student.startMonth || "",
      phone: student.phone || "",
      email: student.email || "",
      weekdays: Array.isArray(student.weekdays) ? student.weekdays : [],
      saturdaySlots: Array.isArray(student.saturdaySlots) ? student.saturdaySlots : [],
    })),
    attendance: Array.isArray(parsed.attendance) ? parsed.attendance : [],
    billing: Array.isArray(parsed.billing) ? parsed.billing : [],
    handovers: Array.isArray(parsed.handovers) ? parsed.handovers : [],
    incidentalSales: Array.isArray(parsed.incidentalSales) ? parsed.incidentalSales : [],
  };
}
...
function snapshotCounts(snapshotState) {
  return `${snapshotState.classrooms.length}教室 / ${snapshotState.students.length}名 / 出席${snapshotState.attendance.length}件 / 集金${snapshotState.billing.length}件`;
}
  ...
  backupSnapshots = [
    {
      ...
      summary: snapshotCounts(snapshotState),
      payload: JSON.stringify(snapshotState),
    },
    ...backupSnapshots,
...
function saveState(options = {}) {
  const { skipSnapshot = false, label = "自動保存", skipExternalBackup = false } = options;
  localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
  saveMeta.lastSavedAt = new Date().toISOString();
  persistSaveMeta();
  if (!skipSnapshot) createBackupSnapshot(label);
  if (!skipExternalBackup) scheduleExternalBackup();
}

function resetState() {
  const next = normalizeState(seedState);
  Object.keys(state).forEach((key) => delete state[key]);
  Object.assign(state, next);
  saveState({ label: "初期データに戻す" });
}
...
function expandConfiguredDays(days, students) {
  ...
  days.forEach((day) => {
    ...
    SATURDAY_SLOT_ORDER.forEach((slot) => {
      if (students.some((student) => (student.weekdays || []).includes("土") && (student.saturdaySlots || []).includes(slot))) {
        labels.push(`土${slot}`);
      }
    });
    if (students.some((student) => (student.weekdays || []).includes("土") && !(student.saturdaySlots || []).length)) {
      labels.push("土");
    }
  });
  return labels;
...
function findSectionDays(students) {
  ...
  WEEKDAY_ORDER.forEach((weekday) => {
    if (weekday !== "土") {
      if (students.some((student) => (student.weekdays || []).includes(weekday))) days.push(weekday);
      return;
    }
    SATURDAY_SLOT_ORDER.forEach((slot) => {
      if (students.some((student) => (student.weekdays || []).includes("土") && (student.saturdaySlots || []).includes(slot))) {
...
function sectionStudentsByDay(students, days) {
  return days.map((weekday) => ({
    weekday,
    students: students
      .filter((student) => studentMatchesDayLabel(student, weekday))
...
function buildStudentListSections() {
  ...
  const regularStudents = activeStudents.filter((student) => !isArtStudent(student));
  const artStudents = activeStudents.filter((student) => isArtStudent(student));
  const sections = [];
  const usedClassrooms = new Set();

  STUDENT_LIST_SECTION_CONFIG.forEach((config) => {
    const sectionStudents = regularStudents.filter((student) => config.classroomNames.includes(getClassroom(student.classroomId)?.name || ""));
    ...
    sections.push({
      title: config.title,
      type: "regular",
      dayGroups: sectionStudentsByDay(sectionStudents, days),
    });
  });

  const unmatchedStudents = regularStudents.filter((student) => !usedClassrooms.has(getClassroom(student.classroomId)?.name || ""));
  const unmatchedByClassroom = unmatchedStudents.reduce((map, student) => {
    const classroomName = getClassroom(student.classroomId)?.name || "未設定教室";
    if (!map.has(classroomName)) map.set(classroomName, []);
    map.get(classroomName).push(student);
    return map;
  }, new Map());
  ...
  unmatchedByClassroom.forEach((students, classroomName) => {
    sections.push({
      title: classroomName,
      type: "regular",
      dayGroups: sectionStudentsByDay(students, findSectionDays(students)),
    });
  });
  ...
  if (artStudents.length) {
    sections.push({
      ...
      dayGroups: ART_CLASS_NAMES.map((className) => ({
        weekday: className,
        students: artStudents
          .filter((student) => studentClassName(student) === className)
          .sort((a, b) => a.name.localeCompare(b.name, "ja")),
...
function weekdayTotals(students) {
  ...
  WEEKDAY_ORDER.forEach((weekday) => {
    if (weekday !== "土") {
      totals.push({
        weekday,
        count: students.filter((student) => (student.weekdays || []).includes(weekday)).length,
      });
      return;
    }
    SATURDAY_SLOT_ORDER.forEach((slot) => {
      totals.push({
        weekday: `土${slot}`,
        count: students.filter((student) => (student.weekdays || []).includes("土") && (student.saturdaySlots || []).includes(slot)).length,
      });
    });
    const noSlotCount = students.filter((student) => (student.weekdays || []).includes("土") && !(student.saturdaySlots || []).length).length;
    if (noSlotCount) {
      totals.push({ weekday: "土", count: noSlotCount });
    }
  });
  return totals;
}
...
function buildStudentListExcelXml() {
  ...
  const maxNames = Math.max(
    6,
    ...sections.flatMap((section) => section.dayGroups.map((group) => group.students.length)),
  );
  const totalColumns = 3 + maxNames;
  const rows = [];
  const legendItems = [
    ...Object.entries(classStyleMap).map(([label, style]) => ({ label, styleId: style.styleId })),
  ];
  ...
  sections.forEach((section) => {
    ...
    visibleGroups.forEach((group, index) => {
      ...
      group.students.forEach((student) => {
        rowCells.push(xmlCell(student.name, classColorStyleId(studentClassName(student))));
      });
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Styles>
  <Style ss:ID="Default" ss:Name="Normal">
   ...
   <Borders/>
   <Font ss:FontName="Meiryo" ss:Size="10"/>
   <Interior/>
   <NumberFormat/>
   <Protection/>
  </Style>
 <Style ss:ID="styleTitle">
   ...
   <Font ss:FontName="Meiryo" ss:Size="16" ss:Bold="1"/>
   <Interior ss:Color="#D6E8FF" ss:Pattern="Solid"/>
  </Style>
  <Style ss:ID="styleMeta">
   <Font ss:FontName="Meiryo" ss:Size="9"/>
  </Style>
  <Style ss:ID="styleSection">
   <Alignment ss:Vertical="Center"/>
   <Font ss:FontName="Meiryo" ss:Size="11" ss:Bold="1"/>
   <Interior ss:Color="#BBD7F5" ss:Pattern="Solid"/>
  </Style>
  <Style ss:ID="styleCategory">
   <Alignment ss:Vertical="Center"/>
   <Borders>
   ...
   <Interior ss:Color="#F3F7FB" ss:Pattern="Solid"/>
  </Style>
  <Style ss:ID="styleDayLabel">
   <Alignment ss:Horizontal="Center" ss:Vertical="Center"/>
   <Font ss:FontName="Meiryo" ss:Size="10" ss:Bold="1"/>
   <Borders>
    <Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1"/>
    <Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1"/>
    <Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1"/>
   ...
   <Interior ss:Color="#E9F3FF" ss:Pattern="Solid"/>
  </Style>
  ...
  <Style ss:ID="styleHeaderCompact">
   <Alignment ss:Horizontal="Center" ss:Vertical="Center"/>
   <Font ss:FontName="Meiryo" ss:Size="9" ss:Bold="1"/>
   <Borders>
    <Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1"/>
    <Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1"/>
    <Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1"/>
  ...
  <Style ss:ID="styleCountStrong">
   ...
   <Interior ss:Color="#FFF8D6" ss:Pattern="Solid"/>
  </Style>
  <Style ss:ID="styleNameDefault">
   <Borders>
    <Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1"/>
    <Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1"/>
    <Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1"/>
  ...
  ${Object.values(classStyleMap).map((style) => `
  <Style ss:ID="${style.styleId}">
   <Alignment ss:Horizontal="Center" ss:Vertical="Center"/>
   <Borders>
    <Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1"/>
    <Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1"/>
  ...
  <Style ss:ID="styleLegendTitle">
   <Font ss:FontName="Meiryo" ss:Size="10" ss:Bold="1"/>
   <Interior ss:Color="#DCEAFE" ss:Pattern="Solid"/>
  </Style>
  <Style ss:ID="styleLegendSpacer">
   <Interior ss:Color="#FFFFFF" ss:Pattern="Solid"/>
  </Style>
  <Style ss:ID="styleTotalLabel">
   <Alignment ss:Horizontal="Right" ss:Vertical="Center"/>
   <Font ss:FontName="Meiryo" ss:Bold="1"/>
   <Interior ss:Color="#DCEAFE" ss:Pattern="Solid"/>
  </Style>
  <Style ss:ID="styleTotalValue">
 ...
 <Worksheet ss:Name="生徒一覧">
  ...
  <WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel">
   <PageSetup>
    ...
    <Header x:Margin="0.3"/>
    <Footer x:Margin="0.3"/>
    <PageMargins x:Bottom="0.5" x:Left="0.3" x:Right="0.3" x:Top="0.5"/>
   </PageSetup>
   <Print>
    <ValidPrinterInfo/>
...
function buildSalesReportExcelXml() {
  ...
  const summaryRows = [
    `<Row>${xmlCell("年度", "styleHeader")}${xmlCell("対象月", "styleHeader")}${xmlCell("請求額", "styleHeader")}${xmlCell("入金額", "styleHeader")}${xmlCell("未入金額", "styleHeader")}${xmlCell("入金件数", "styleHeader")}${xmlCell("未入金件数", "styleHeader")}</Row>`,
    ...(monthlyRows.length
      ? monthlyRows.map((item) => `<Row>${xmlCell(fiscalYearLabel(item.month), "styleCell")}${xmlCell(monthLabel(item.month), "styleCell")}${xmlCell(item.billed, "styleMoney", "Number")}${xmlCell(item.paid, "styleMoney", "Number")}${xmlCell(Math.max(item.billed - item.paid, 0), "styleMoney", "Number")}${xmlCell(item.paidCount, "styleCount", "Number")}${xmlCell(item.unpaidCount, "styleCount", "Number")}</Row>`)
      : [`<Row>${xmlCell("データなし", "styleCell", "String", 6)}</Row>`]),
  ].join("");
  const detailByMonth = monthlySalesDetailRowsByMonth();
  const detailSheets = Object.entries(detailByMonth)
    ...
    .map(([month, rows]) => {
      ...
      return `
 <Worksheet ss:Name="${escapeXml(sheetNameForMonth(month))}">
  <Table ss:ExpandedColumnCount="4" ss:ExpandedRowCount="${rows.length + 3}">
   <Column ss:Width="90"/><Column ss:Width="150"/><Column ss:Width="240"/><Column ss:Width="90"/>
   ${detailRows}
  </Table>
 </Worksheet>`;
    })
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Styles>
  ...
  <Style ss:ID="styleTitle">
   <Alignment ss:Horizontal="Center" ss:Vertical="Center"/>
   <Font ss:FontName="Meiryo" ss:Size="15" ss:Bold="1"/>
   <Interior ss:Color="#DCEAFE" ss:Pattern="Solid"/>
  </Style>
  <Style ss:ID="styleSectionTitle">
...
function getStudentsByClassroom(classroomId) {
  return state.students.filter((student) => classroomId === "all" || student.classroomId === classroomId);
}

function getUsersByClassroom(classroomId) {
  return state.users.filter((user) => classroomId === "all" || user.classroomId === classroomId || user.classroomId === "all");
}
...
function fiscalYearLabel(month) {
  return `${fiscalYearForMonth(month)}年度`;
}
...
function monthlySalesRows() {
  return trackedMonths().map((month) => {
    const students = state.students.filter((student) => (
      ...
      isStudentActiveFromMonth(student, month)
    ));
    const summaries = students.map((student) => billingSummaryForStudent(student.id, month));
...
function monthlySalesDetailRowsByMonth() {
  return Object.fromEntries(trackedMonths().map((month) => {
    const studentRows = state.students
      ...
      .map((student) => {
        const summary = billingSummaryForStudent(student.id, month);
        return {
          month,
          category: "生徒月謝",
          name: student.name,
          content: summary.carryover > 0 ? `月謝（前月繰越 ${yen(summary.carryover)} を含む）` : "月謝",
...
function sheetNameForMonth(month) {
  return `${month.slice(0, 4)}-${month.slice(5, 7)}詳細`;
}
...
function fiscalSalesRows() {
  ...
  monthlySalesRows().forEach((item) => {
    ...
    const summary = fiscalMap.get(key) || {
      fiscalYear: key,
      billed: 0,
      paid: 0,
      unpaid: 0,
...
function activeStudents() {
  return state.students.filter((student) => student.status === "在籍" && matchClassroom(student.classroomId));
}
...
function previousCarryoverForStudent(studentId, month) {
  ...
  return [...monthlyMap.values()]
    .reduce((sum, item) => sum + Math.max(item.billed - item.paid, 0), 0);
}

function previousUnpaidMonthsForStudent(studentId, month) {
  return trackedMonths()
    .filter((itemMonth) => itemMonth < month && billingSummaryForStudent(studentId, itemMonth).outstanding > 0);
}
...
function billingSummaryForStudent(studentId, month = currentMonth(), excludeBillingId = null) {
  ...
  const currentMonthEntries = getBillingEntriesByStudent(studentId)
    .filter((item) => item.month === month && item.id !== excludeBillingId);
...
function expectedTuitionForMonth(classroomId = null) {
  return state.students
    .filter((student) => student.status === "在籍" && (!classroomId || student.classroomId === classroomId))
...
function previousOutstandingAmount(classroomId = null, month = currentMonth()) {
  ...
  return state.students
    .filter((student) => (
      ...
      matchClassroom(student.classroomId) &&
      isStudentActiveFromMonth(student, targetMonth)
    ))
...
async function writeExternalBackupFile(showNotice) {
  ...
  try {
    ...
    await writable.write(JSON.stringify({
      ...
      source: "robot-school-shared-system",
...
function overwriteState(nextState, saveLabel) {
  const normalized = normalizeState(nextState);
  Object.keys(state).forEach((key) => delete state[key]);
  Object.assign(state, normalized);
  saveState({ label: saveLabel });
  setLoginOptions();
  if (currentUser) {
    currentUser = state.users.find((user) => user.id === currentUser.id) || state.users[0] || null;
    if (currentUser) renderApp();
  }
  renderSaveStatus();
}

function restoreBackupSnapshot(snapshotId) {
  const snapshot = backupSnapshots.find((item) => item.id === snapshotId);
  if (!snapshot) {
    showToast("復元点が見つかりません");
    return;
  }

  try {
    overwriteState(JSON.parse(snapshot.payload), `復元: ${snapshot.label}`);
    renderSaveStatus();
...
function renderSaveStatus() {
  ...
  const externalSavedAt = saveMeta.lastExternalSavedAt ? formatDateTime(saveMeta.lastExternalSavedAt) : "未保存";
  const externalLabel = saveMeta.externalFileName
    ? `外部バックアップ: ${saveMeta.externalFileName}`
    : "このブラウザ内に保存";

  if (els.saveLocationLabel) {
    els.saveLocationLabel.textContent = externalLabel;
  }
  if (els.saveLocationDetail) {
    els.saveLocationDetail.textContent =
      `主保存: このブラウザ / 最終保存: ${browserSavedAt} / 推奨保存先: ${RECOMMENDED_BACKUP_PATH} / 外部保存: ${externalSavedAt}`;
  }
  if (els.backupSummary) {
    els.backupSummary.textContent =
      `最終保存は ${browserSavedAt}、外部バックアップは ${saveMeta.externalFileName ? `${saveMeta.externalFileName} に ${externalSavedAt}` : "未設定"} です。`;
  }
}
...
function filteredAttendance() {
  return state.attendance
    .filter((item) => matchClassroom(item.classroomId))
...
function filteredBilling() {
  return state.students
    .filter((student) => student.status === "在籍" && matchClassroom(student.classroomId) && isStudentActiveFromMonth(student, currentMonth()))
    .map((student) => {
      const billingRecord = findBillingRecord(student.id, currentMonth());
      const summary = billingSummaryForStudent(student.id, currentMonth());
      const previousUnpaidMonths = previousUnpaidMonthsForStudent(student.id, currentMonth());
      return {
...
function setCurrentUserInfo() {
  const classroomLabel = currentUser.classroomId === "all"
    ? "全教室を閲覧可能"
    : `${getClassroom(currentUser.classroomId)?.name || "-"} 担当`;
  ...
  els.topbarMeta.textContent = `${currentUser.name}として利用中 / ブラウザ保存 ${formatDateTime(saveMeta.lastSavedAt)} / 外部バックアップ ${saveMeta.externalFileName || "未設定"}`;
}
...
function updateSaturdaySlotVisibility() {
  const saturdayChecked = Array.from(els.studentForm.querySelectorAll('input[name="weekday"]'))
    .some((checkbox) => checkbox.value === "土" && checkbox.checked);
  els.saturdaySlotsFieldset.classList.toggle("hidden", !saturdayChecked);
  if (!saturdayChecked) {
    els.studentForm.querySelectorAll('input[name="saturdaySlot"]').forEach((checkbox) => {
...
function setSelectOptions() {
  const classroomOptions = state.classrooms.map((classroom) => `<option value="${classroom.id}">${classroom.name}</option>`).join("");
  const teacherClassroomOptions =
    `<option value="all">全教室担当</option>` +
    state.classrooms.map((classroom) => `<option value="${classroom.id}">${classroom.name}</option>`).join("");
  const classOptionOptions = [
    `<option value="">選択してください</option>`,
    ...state.classOptions.map((item) => `<option value="${item.id}">${item.name}</option>`),
  ].join("");
  const studentOptions = (classroomId) => {
    const items = getStudentsByClassroom(classroomId || getActiveFilter()).map((student) => (
  ...
  syncStudents(els.studentForm, "");

  attendanceClassroom.onchange = () => {
    syncStudents(els.attendanceForm, attendanceClassroom.value);
    renderAttendance();
  };
  billingClassroom.onchange = () => {
    syncStudents(els.billingForm, billingClassroom.value);
    autofillBillingAmount();
  };
  handoverClassroom.onchange = () => syncStudents(els.handoverForm, handoverClassroom.value);
  els.attendanceForm.querySelector('input[name="date"]').onchange = () => renderAttendance();
}

function autofillBillingAmount() {
  const month = els.billingForm.querySelector('input[name="month"]').value || currentMonth();
  const studentId = els.billingForm.querySelector('select[name="studentId"]').value;
  const billedInput = els.billingForm.querySelector('input[name="billed"]');
  const paidInput = els.billingForm.querySelector('input[name="paid"]');
  if (!studentId) {
    billedInput.value = "";
    paidInput.value = "";
    paidInput.max = "";
  ...
  billedInput.value = Number(summary.expected || 0);
  if (!formEditState.billing) {
    paidInput.value = Number(summary.outstanding || 0);
  }
  paidInput.max = Number(summary.outstanding || 0);
  refreshBillingEditNote();
}

function setBillingQuickActionsVisible(visible) {
  if (!els.billingQuickActions) return;
  els.billingQuickActions.classList.toggle("hidden", !visible);
}

function refreshBillingEditNote() {
  if (!els.billingEditNote) return;
  ...
  const duplicate = state.billing.find((entry) => (
    entry.studentId === studentId &&
    entry.month === month &&
    entry.id !== editingId
  ));
  els.billingEditNote.classList.remove("hidden");
  els.billingEditNote.innerHTML = `
    登録中の内容: 請求額 ${yen(item.billed)} / 入金額 ${yen(item.paid)} / 対象月 ${monthLabel(item.month)}
    <br>編集中の対象: ${monthLabel(month)} / この月の請求見込み ${yen(summary.expected)} / 未収上限 ${yen(summary.outstanding)}
    ${duplicate ? `<br>注意: ${monthLabel(month)} には別の集金登録がすでにあります。` : ""}
  `;
...
function renderDashboard() {
  ...
  const summaries = getAccessibleClassrooms()
    .filter((classroom) => currentFilter === "all" || classroom.id === currentFilter)
    .map(summarizeClassroom);
  ...
  els.latestHandoverList.innerHTML = openHandovers.slice(0, 6).map((item) => {
    ...
    return `
      <li>
        ...
        <p>${classroom} / ${student} / ${item.body}</p>
      </li>
    `;
  }).join("");
}
...
function renderAttendance() {
  ...
  const scheduledStudents = state.students
    .filter((student) => (
      student.status === "在籍" &&
      student.classroomId === selectedClassroomId &&
      (student.weekdays || []).includes(weekday)
    ))
    .sort((a, b) => a.name.localeCompare(b.name, "ja"));
  ...
  const scheduledAttendanceMap = new Map(
    state.attendance
      .filter((item) => item.date === selectedDate && item.classroomId === selectedClassroomId)
  ...
  const billingSummaryText = (studentId) => {
    ...
    if (previousUnpaidMonths.length) {
      return `未収 ${yen(billing.outstanding)} / 繰越 ${previousUnpaidMonths.map(monthLabel).join(" / ")}`;
    }
    return `${billing.status} / 今月未収 ${yen(billing.outstanding)}`;
  };
  const renderScheduledCard = (student) => {
    const classOption = getClassOption(student.classOptionId)?.name || "-";
    const attendanceDaysThisMonth = state.attendance.filter((attendanceItem) => (
      attendanceItem.studentId === student.id &&
      monthOf(attendanceItem.date) === currentMonth() &&
      attendanceItem.status === "出席"
    )).length;
    const attendanceRecord = scheduledAttendanceMap.get(student.id);
    const billing = billingSummaryForStudent(student.id, currentMonth());
    return `
      <article class="scheduled-card">
        <strong>${student.name}</strong>
        <p>${student.guardian || "保護者情報未登録"}</p>
        <p>クラス: ${classOption} / 当月出席: ${attendanceDaysThisMonth}日</p>
        <p>集金: <span class="scheduled-status ${pillForStatus(billing.status)}">${billingSummaryText(student.id)}</span></p>
        <span>${weekdaysText(student)}</span>
        <div class="scheduled-actions">
          <button
            class="primary-btn scheduled-action-btn"
            data-mark-attendance="${student.id}"
            ${attendanceRecord?.status === "出席" ? "disabled" : ""}
          >${attendanceRecord?.status === "出席" ? "出席済み" : "出席登録"}</button>
          <button class="ghost-inline-btn scheduled-action-btn" data-open-billing="${student.id}">
            ${findBillingRecord(student.id, currentMonth()) ? "集金編集" : "集金登録"}
          </button>
        </div>
      </article>
    `;
  ...
  if (!scheduledStudents.length) {
    ...
    const saturdayGroups = SATURDAY_SLOT_ORDER
      .map((slot) => ({
        label: `${slot}`,
        students: scheduledStudents.filter((student) => (student.saturdaySlots || []).includes(slot)),
      }))
      .filter((group) => group.students.length);
    const noSlotStudents = scheduledStudents.filter((student) => !(student.saturdaySlots || []).length);
    if (noSlotStudents.length) {
      saturdayGroups.push({ label: "時間未設定", students: noSlotStudents });
    }
    els.scheduledStudentsList.innerHTML = saturdayGroups.map((group) => `
      <section class="scheduled-slot-group">
        <h4>${group.label}</h4>
        <div class="scheduled-slot-cards">
          ${group.students.map(renderScheduledCard).join("")}
        </div>
      </section>
    `).join("");
  } else {
...
function markAttendanceFromSchedule(studentId) {
  ...
  const existingRecord = state.attendance.find((item) => (
    item.date === date &&
...
function renderBilling() {
  els.billingBody.innerHTML = filteredBilling().map((item) => {
    const student = getStudent(item.studentId)?.name || "-";
    const previousUnpaidLabel = item.previousUnpaidMonths.length
      ? `${item.previousUnpaidMonths.map(monthLabel).join(" / ")} (${yen(item.previousCarryover)})`
      : "-";
    return `
      <tr>
        <td>${item.month}</td>
...
function renderSales() {
  ...
  const currentMonthRow = monthlyRows.find((item) => item.month === currentMonth()) || {
    billed: 0,
    paid: 0,
    unpaid: 0,
    paidCount: 0,
  ...
  const currentFiscalRow = fiscalRows.find((item) => item.fiscalYear === fiscalYearForMonth(currentMonth())) || {
    billed: 0,
    paid: 0,
    unpaid: 0,
    paidCount: 0,
  ...
  els.salesMonthlyBody.innerHTML = monthlyRows.length
    ? monthlyRows.map((item) => `
      <tr>
        ...
        <td>${yen(item.billed)}</td>
        <td>${yen(item.paid)}</td>
        <td>${yen(item.unpaid)}</td>
        <td>${item.paidCount}件</td>
        <td>${item.unpaidCount}件</td>
      </tr>
    `).join("")
...
function renderMaster() {
  ...
  els.teacherTableBody.innerHTML = getUsersByClassroom(currentFilter === "all" ? "all" : currentFilter)
    .map((user) => `
      <tr>
        <td>${user.name}</td>
        <td>${user.email || "-"}</td>
  ...
  els.incidentalSaleTableBody.innerHTML = state.incidentalSales
  ...
  els.billingIncidentalSaleTableBody.innerHTML = state.incidentalSales
    .filter((item) => item.month === currentMonth())
    .slice()
    .sort((a, b) => b.month.localeCompare(a.month))
    .map((item) => `
  ...
  els.studentTableBody.innerHTML = state.students
    .filter((student) => matchClassroom(student.classroomId))
    .map((student) => {
      const billing = billingSummaryForStudent(student.id, currentMonth());
      const overallBilling = overallBillingSummaryForStudent(student.id);
      return `
        <tr>
          <td>${student.name}</td>
          <td>${getClassOption(student.classOptionId)?.name || "-"}</td>
          <td><span class="pill ${student.status === "在籍" ? "ok" : "neutral"}">${student.status}</span></td>
          ...
          <td>${weekdaysText(student)}</td>
          <td>${student.guardian || "-"}</td>
          <td>${student.phone || "-"}</td>
          <td>${student.email || "-"}</td>
          <td class="row-actions">
            <button class="link-btn" data-edit-student-billing="${student.id}">集金編集</button>
            <button class="link-btn" data-edit-student="${student.id}">編集</button>
            <button class="link-btn" data-delete-student="${student.id}">削除</button>
          </td>
        </tr>
      `;
    }).join("");
...
function setFormMode(type, editing, submitText) {
  ...
  const submitMap = {
    classOption: els.classOptionSubmitBtn,
    incidentalSale: els.incidentalSaleSubmitBtn,
    attendance: els.attendanceSubmitBtn,
    billing: els.billingSubmitBtn,
    handover: els.handoverSubmitBtn,
    classroom: els.classroomSubmitBtn,
    teacher: els.teacherSubmitBtn,
    student: els.studentSubmitBtn,
  };
  const cancelMap = {
    classOption: els.classOptionCancelBtn,
    incidentalSale: els.incidentalSaleCancelBtn,
    attendance: els.attendanceCancelBtn,
    billing: els.billingCancelBtn,
    handover: els.handoverCancelBtn,
    classroom: els.classroomCancelBtn,
    teacher: els.teacherCancelBtn,
    student: els.studentCancelBtn,
  };
...
function clearClassOptionForm() {
  ...
  const sortOrderInput = els.classOptionForm.querySelector('input[name="sortOrder"]');
  sortOrderInput.value = state.classOptions.length + 1;
...
function setIncidentalSaleTemplateOptions() {
  const templateOptions = [
    `<option value="">選択してください</option>`,
    ...state.incidentalSales
      .slice()
      .sort((a, b) => b.month.localeCompare(a.month))
      .map((item) => `<option value="${item.id}">${monthLabel(item.month)} / ${item.name} / ${item.content} / ${yen(item.amount)}</option>`),
  ].join("");

  if (els.incidentalSaleTemplateSelect) {
    els.incidentalSaleTemplateSelect.innerHTML = templateOptions;
  }
  if (els.billingIncidentalSaleTemplateSelect) {
    els.billingIncidentalSaleTemplateSelect.innerHTML = templateOptions;
  }
}
...
function clearStudentForm() {
  ...
  els.studentForm.querySelectorAll('input[name="weekday"]').forEach((checkbox) => {
    checkbox.checked = false;
  });
  els.studentForm.querySelectorAll('input[name="saturdaySlot"]').forEach((checkbox) => {
    checkbox.checked = false;
...
function submitIncidentalSaleForm(event) {
...
function startEditStudent(id) {
  ...
  els.studentForm.querySelectorAll('input[name="weekday"]').forEach((checkbox) => {
    checkbox.checked = (item.weekdays || []).includes(checkbox.value);
  });
  els.studentForm.querySelectorAll('input[name="saturdaySlot"]').forEach((checkbox) => {
    checkbox.checked = (item.saturdaySlots || []).includes(checkbox.value);
  });
  updateSaturdaySlotVisibility();
...
function deleteClassroomCascade(classroomId) {
  ...
  state.users = state.users.filter((user) => user.classroomId !== classroomId);
  state.students = state.students.filter((student) => student.classroomId !== classroomId);
...
function activateView(viewName) {
  ...
  Object.entries(els.views).forEach(([key, node]) => {
    node.classList.toggle("is-visible", key === viewName);
  });
  document.querySelectorAll(".nav-item").forEach((button) => {
    button.classList.toggle("is-active", button.dataset.view === viewName);
  });
  els.viewTitle.textContent = viewTitles[viewName];
  if (currentSubView[viewName]) {
    activateSubView(viewName, currentSubView[viewName]);
  }
}
...
function bindEvents() {
  ...
  els.subviewButtons.forEach((button) => {
    button.addEventListener("click", () => {
      activateSubView(button.dataset.section, button.dataset.subview);
    });
  });
  ...
  els.studentForm.querySelectorAll('input[name="weekday"]').forEach((checkbox) => {
    checkbox.addEventListener("change", updateSaturdaySlotVisibility);
  });
  els.incidentalSaleTemplateSelect.addEventListener("change", (event) => {
    if (event.target.value) applyIncidentalSaleTemplate(event.target.value);
  });
  els.billingIncidentalSaleTemplateSelect.addEventListener("change", (event) => {
    if (event.target.value) applyIncidentalSaleTemplate(event.target.value);
  });
  els.billingForm.querySelector('input[name="month"]').addEventListener("change", () => {
    autofillBillingAmount();
  });
  els.billingForm.querySelector('select[name="studentId"]').addEventListener("change", () => {
    autofillBillingAmount();
  });
  els.billingForm.querySelector('input[name="paid"]').addEventListener("input", () => {
    refreshBillingEditNote();
  });
  els.billingMarkUnpaidBtn.addEventListener("click", () => {
    markBillingFormAsUnpaid();
  });
  els.billingMoveCurrentMonthBtn.addEventListener("click", () => {
    moveBillingFormToCurrentMonth();
  });
  els.billingFillPaidBtn.addEventListener("click", () => {
    fillBillingFormAsPaid();
  });
  ...
  els.globalFilter.addEventListener("change", () => {
    currentFilter = els.globalFilter.value;
    renderApp();
  });
  ...
  els.quickAddBtn.addEventListener("click", () => {
    ...
    if (currentSubView[targetView]) {
      activateSubView(targetView, "entry");
    }
  });

  els.logoutBtn.addEventListener("click", logout);
  els.chooseBackupFileBtn.addEventListener("click", () => {
    chooseBackupFile();
  });
  els.saveBackupFileBtn.addEventListener("click", () => {
    writeExternalBackupFile(true);
  });
  ...
  els.billingForm.addEventListener("submit", (event) => {
    ...
    if (!studentId) {
      showToast("生徒を選択してください");
      return;
    }

    const expectedSummary = billingSummaryForStudent(studentId, month, formEditState.billing);
    const duplicate = state.billing.find((item) => (
      item.studentId === studentId &&
      item.month === month &&
      item.id !== formEditState.billing
    ));

    if (duplicate) {
      showToast("この生徒の対象月の集金はすでにあります。既存の編集画面を開きます");
      startEditBilling(duplicate.id);
      return;
    }

    if (paid > expectedSummary.outstanding) {
      showToast(`入金額は不足額 ${yen(expectedSummary.outstanding)} までにしてください`);
      return;
    }
    ...
    const payload = {
      ...
      classroomId: formData.get("classroomId"),
      studentId,
      billed: expectedSummary.expected,
      paid,
  ...
  els.handoverForm.addEventListener("submit", (event) => {
    event.preventDefault();
    const formData = new FormData(event.currentTarget);
    const payload = {
  ...
  els.classOptionForm.addEventListener("submit", (event) => {
    event.preventDefault();
    const formData = new FormData(event.currentTarget);
    const name = String(formData.get("name")).trim();
    if (state.classOptions.some((item) => item.name === name && item.id !== formEditState.classOption)) {
      showToast("同じクラス名がすでに登録されています");
      return;
    }
  ...
  els.classroomForm.addEventListener("submit", (event) => {
    event.preventDefault();
    const formData = new FormData(event.currentTarget);
    const payload = {
  ...
  els.teacherForm.addEventListener("submit", (event) => {
    ...
    if (state.users.some((user) => user.email?.toLowerCase() === email && user.id !== formEditState.teacher)) {
      showToast("同じメールアドレスの先生がすでに登録されています");
      return;
    }
    ...
    if (formEditState.teacher) {
      state.users = state.users.map((user) => user.id === formEditState.teacher ? payload : user);
  ...
  els.studentForm.addEventListener("submit", (event) => {
    ...
    if (formEditState.student) {
      state.students = state.students.map((student) => student.id === formEditState.student ? payload : student);
    } else {
  ...
  document.body.addEventListener("click", (event) => {
    ...
    if (markAttendanceStudentId) {
      markAttendanceFromSchedule(markAttendanceStudentId);
      return;
    }
    if (openBillingStudentId) {
      startStudentBillingEdit(openBillingStudentId, currentMonth());
      return;
    }
    if (editAttendanceId) {
      startEditAttendance(editAttendanceId);
      return;
    }
    if (editBillingId) {
      startEditBilling(editBillingId);
      return;
    }
    if (editHandoverId) {
      startEditHandover(editHandoverId);
      return;
    }
    if (editClassOptionId) {
      startEditClassOption(editClassOptionId);
      return;
    }
    if (editIncidentalSaleId) {
      startEditIncidentalSale(editIncidentalSaleId);
      return;
    }
    if (reuseIncidentalSaleId) {
      startReuseIncidentalSale(reuseIncidentalSaleId);
      return;
    }
    if (editClassroomId) {
      startEditClassroom(editClassroomId);
      return;
    }
    if (editUserId) {
      startEditTeacher(editUserId);
      return;
    }
    if (editStudentId) {
      startEditStudent(editStudentId);
      return;
    }
    if (editStudentBillingId) {
      startStudentBillingEdit(editStudentBillingId);
      return;
    }
    if (restoreBackupId) {
      restoreBackupSnapshot(restoreBackupId);
      return;
    }
    ...
    if (classOptionId) {
      if (state.students.some((student) => student.classOptionId === classOptionId)) {
        showToast("生徒に使われているクラスは削除できません");
        return;
      }
    ...
    if (classroomId) {
      if (currentUser?.classroomId === classroomId) {
        showToast("ログイン中の担当教室は削除できません");
        return;
      }
    ...
    if (userId) {
      if (currentUser?.id === userId) {
        showToast("ログイン中の先生アカウントは削除できません");
        return;
      }
  ...
  els.importInput.addEventListener("change", async (event) => {
    ...
    try {
      overwriteState(JSON.parse(text), `JSON読込: ${file.name}`);
      renderSaveStatus();
